...
export const TenantManagementView: React.FC<TenantManagementViewProps> = ({ onOpenRoomModal }) => {
  ...
  // Counts
  const occupiedRooms = rooms.filter(r => r.status === 'terisi' || r.status === 'menunggu_pembayaran');
  const emptyRooms = rooms.filter(r => r.status === 'kosong');
  const activeTenants = rooms.filter(r => r.tenant).map(r => ({ ...r.tenant!, roomNumber: r.roomNumber, roomType: r.type, basePrice: r.basePrice }));
  ...
  const handleOpenCheckInModal = (booking?: RentalBooking, preselectedRoomId?: number) => {
    // Find first empty room as best default
  ...
  const handleWhatsAppTenant = (phone: string, name: string, roomNum: string) => {
    ...
    const text = `Halo ${name} (${roomNum} ${settings.kostName}),%0A%0AInformasi pengelola kos:%0A...`;
  ...
  const filteredAllTenants = tenants.filter(t => {
  ...
  return (
    <div id="tenant-management-view" className="space-y-6 animate-in fade-in duration-200">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Data tersinkronisasi langsung ke Google Cloud Firestore ({isCloudConnected ? '🟢 Live Connected' : '🟡 Offline Mode'})
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5 pt-5">
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5">
            ...
            <div className="text-xl sm:text-2xl font-black text-slate-900 mt-1">
              {occupiedRooms.length} <span className="text-xs font-normal text-slate-500">/ 8 Kamar</span>
            </div>
            <div className="text-[10px] text-emerald-600 font-bold mt-1">
              Tingkat Hunian: {Math.round((occupiedRooms.length / 8) * 100)}%
            </div>
          </div>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5">
            ...
            <div className="text-[10px] text-slate-500 mt-1">
              {emptyRooms.map(r => r.roomNumber).join(', ') || 'Semua Terisi'}
            </div>
          </div>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5">
            ...
            <div className="text-xl sm:text-2xl font-black text-blue-600 mt-1">
              {tenants.length} <span className="text-xs font-normal text-slate-500">Penghuni</span>
            </div>
            <div className="text-[10px] text-slate-500 mt-1">
              {activeTenants.length} Aktif &bull; {tenants.filter(t => t.status === 'checked_out').length} Riwayat/Checkout
            </div>
          </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-xs">
        <div className="border-b border-slate-200 px-4 sm:px-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3 py-3 bg-slate-50/50">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            >
              ...
              <span>Penghuni Aktif 8 Kamar ({activeTenants.length})</span>
            </button>
            ...
            >
              ...
              <span>Database Semua Penyewa ({tenants.length})</span>
            </button>
        ...
        {/* Tab 1: Status 8 Kamar & Penghuni Aktif */}
        {activeTab === 'tenants' && (
          <div className="p-4 sm:p-6 space-y-6">
            {/* Quick Room Slots Overview */}
            <div>
              <div className="flex items-center justify-between mb-3">
                <h4 className="text-xs font-extrabold text-slate-700 uppercase tracking-wider">
                  Status Alokasi 8 Pintu Kamar Kos
                </h4>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                        ...
                        {room.tenant ? 'Terisi' : 'Siap Huni'}
                      </span>
                    </div>

                    {room.tenant ? (
                      <div>
                        <div className="font-bold text-xs text-slate-800 truncate">{room.tenant.name}</div>
                        <div className="text-[11px] text-slate-500 truncate">{room.tenant.phone}</div>
                        <div className="text-[10px] text-slate-400 mt-1">
                          Masuk: {formatIndonesianDate(room.tenant.checkInDate)}
                        </div>
                      </div>
                    ) : (
                    ...
                    <div className="pt-2 border-t border-slate-200/60 flex items-center justify-between gap-1">
                      ...
                      {room.tenant ? (
                        ...
                        <button
                          onClick={() => handleOpenCheckInModal(undefined, room.id)}
                          className="px-2 py-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-[10px] font-bold cursor-pointer transition shadow-2xs"
                        >
                          + Check-In Kamar Ini
                        </button>
                      )}
                      ...
                      {filteredTenants.map((tenant) => (
                        <tr key={tenant.id} className="hover:bg-slate-50/70 transition">
                          <td className="py-3.5 px-3.5">
                            ...
                            <div className="text-[10px] text-slate-400">{formatRupiah(tenant.basePrice)}/bln</div>
                          </td>
                          <td className="py-3.5 px-3.5">
