...
export const GoogleAuthModal: React.FC<AuthModalProps> = ({
  ...
  const currentRoom = rooms.find(r => r.id === tenantRoomId) || rooms[0];
        ...
        <div className="p-6 space-y-5">
          ...
          {selectedRoleTab === 'penghuni' && (
            <div className="space-y-4">
              ...
              <form onSubmit={handleTenantDirectAccess} className="space-y-3">
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1.5">
                    Pilih Nomor Kamar Anda:
                  </label>
                  <select
                    id="select-tenant-login-room"
                    value={tenantRoomId}
                    onChange={e => setTenantRoomId(Number(e.target.value))}
                  ...
                  >
                    {rooms.map(room => (
                      <option key={room.id} value={room.id}>
                        {room.roomNumber} - {room.type} ({room.tenant ? room.tenant.name : 'Belum Ada Penghuni'})
                      </option>
                    ))}
                  </select>
                </div>
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 flex items-center gap-3">
                    ...
                    <div className="overflow-hidden">
                      ...
                      <div className="text-[11px] text-slate-500">
                        {currentRoom.tenant.phone} &bull; {currentRoom.tenant.occupation}
                      </div>
                    </div>
                  </div>
                )}
                ...
                <button
                  type="submit"
                  id="btn-submit-tenant-portal-login"
                  className="w-full py-3 px-4 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs sm:text-sm transition flex items-center justify-center gap-2 cursor-pointer shadow-xs"
                >
                  <UserCheck className="w-4 h-4" />
                  <span>Buka Portal {currentRoom.roomNumber}</span>
                  <ArrowRight className="w-3.5 h-3.5" />
                </button>
              </form>
              ...
              <div className="relative flex py-1 items-center">
                ...
                <span className="flex-shrink mx-3 text-[11px] text-slate-400">atau login akun Google</span>
                <div className="flex-grow border-t border-slate-200" />
              </div>
              ...
              <button
                ...
                disabled={isSigningIn || isAuthLoading}
                className="w-full py-2.5 px-4 rounded-xl border border-slate-300 hover:bg-slate-50 text-slate-700 font-semibold text-xs transition flex items-center justify-center gap-2 cursor-pointer"
              >
